...
Runway::Runway(std::array<Coordinates, 4> coords, Coordinates center_ref,
  ...
  for (size_t i = 0; i < d.triangles.size(); i += 3) {
    Vector2 x1 = {(float)d.coords[2 * d.triangles[i]],
...
void Runway::draw() {
  for (size_t i = 0; i < this->verties.size(); i += 3) {
    DrawTriangle(this->verties[i], this->verties[i + 1], this->verties[i + 2],
                 this->color);
  }
}
